...
function loadData(id) {
    $.ajax({
        type: 'POST',
        data: { "id": id },
        url: '/TangGiamLuong/Index',
        success: function (response) {
            $("#id1").val(response.MaTangGiamLuong);
            $("#nt1").val(response.NgayThang);
            $("#manv1").val(response.MaNV);
            $("#tkn1").val(response.TaiKhoanNo);
            $("#tkc1").val(response.TaiKhoanChinh);
            $("#st1").val(response.SoTien);
            $("#tt1").val(response.ThongTin);
        },
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status == true) {
                ...
                setTimeout(function () {
                    window.location.replace("/Administrator/TangGiamLuong/Index");
                }, 1000)
            } else {
...
function xoaTangGiamLuong() {
    let id = $("#delete-id").val();
    $.ajax({
        type: 'POST',
        data: { "id": id },
        url: '/TangGiamLuong/Delete',
        success: function (response) {
            if (response.status == true) {
                $("#row-" + id).remove();
